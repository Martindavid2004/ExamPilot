...
function appReducer(state, action) {
  switch (action.type) {
    ...
    case 'ADD_EXAM_FOLDER':
      ...
      if (action.payload && action.payload.folderName && state.examFolders.some(f => f && f.folderName && f.folderName.toLowerCase() === action.payload.folderName.toLowerCase())) {
        return {
          ...state,
          examFolders: state.examFolders.map(f =>
            f && f.folderName && f.folderName.toLowerCase() === action.payload.folderName.toLowerCase() ? action.payload : f
          )
        };
      }
...
const Homepage = () => {
      ...
      if (auth) {
        try {
          ...
          if (response.ok) {
            ...
            const storedTimetables = JSON.parse(localStorage.getItem('examTimetables') || '[]');
  ...
  const handleClearAllTimetables = () => {
    if (window.confirm('This will remove ALL stored timetables locally. Continue?')) {
  ...
  const handleCreateTimetable = async (timetableData) => {
    ...
    if (auth) {
      try {
        // First, check if folder exists or create one
        let folderId = null;
        const folderName = timetableData.tableName;
        
        // Check existing folders - add null checks
        const existingFolder = appState.examFolders.find(f => f && f.folderName === folderName);
        ...
        if (existingFolder) {
          ...
          const folderResponse = await fetch(`${API_BASE_URL}/api/exam-folders`, {
            ...
            body: JSON.stringify({
              ...
              description: `Exam folder for ${folderName}`,
              timetables: []
            }),
          });
        ...
        const nameIndex = storedFolders.findIndex(f => f && f.folderName && folder.folderName && f.folderName.toLowerCase() === folder.folderName.toLowerCase());
        if (nameIndex >= 0) {
          storedFolders[nameIndex] = folder;
        } else {
          storedFolders.push(folder);
        }
        localStorage.setItem('examFolders', JSON.stringify(storedFolders));
        return { success: false, message: `Failed to save to database: ${error.message}. Saved locally instead.` };
      }
    } else {
      ...
      localStorage.setItem('examFolders', JSON.stringify(storedFolders));
      return { success: false, message: 'Not authenticated. Saved locally only.' };
    }
  };
    ...
    if (auth && folder && updatedTimetableData.id) {
      try {
        ...
        if (response.ok) {
          ...
          const timetableIndex = updatedFolder.timetables.findIndex(t => t.id === updatedTimetableData.id);
          if (timetableIndex !== -1) {
      ...
      if (auth) {
        try {
          ...
          if (response.ok) {
            ...
            const storedFolders = JSON.parse(localStorage.getItem('examFolders') || '[]');
            if (updatedFolder.timetables.length === 0) {
              const updatedStoredFolders = storedFolders.filter(f => f.id !== folder.id);
  ...
  const handleDeleteTimetableLocal = (folder, timetableId) => {
    const updatedFolder = { ...folder };
    updatedFolder.timetables = updatedFolder.timetables.filter(t => t.id !== timetableId);
    updatedFolder.updatedAt = new Date().toISOString();
...
const LoginScreen = ({ onLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    alert('Invalid credentials. Please use username: admin, password: password123');
  };
    ...
    <div className="login-screen">
      <div className="login-card">
        ...
        <div style={{ 
          ...
          <h3 style={{ color: '#fbbf24', margin: '0 0 0.5rem 0', fontSize: '0.9rem' }}>
            🔑 Test Credentials
          </h3>
          <p style={{ color: '#d1d5db', margin: '0.25rem 0', fontSize: '0.8rem' }}>
      ...
      default:
        return (
          <div className="dashboard-main">
            ...
            <p className="dashboard-main-subtitle">Welcome, Staff! Select an option from the menu.</p>
    ...
    <div className="dashboard">
      <header className="dashboard-header">
        ...
        <div className="dashboard-nav">
          <button onClick={() => setPage('dashboard')} className="nav-button nav-button-primary">
            Home
          </button>
          <button onClick={() => setPage('create-exam')} className="nav-button nav-button-primary">
            Create Exam
          </button>
          <button onClick={() => setPage('upcoming-exams')} className="nav-button nav-button-primary">
            Exam Timetables
          </button>
          <button onClick={() => setPage('completed-exams')} className="nav-button nav-button-primary">
            Completed Exams
          </button>
...
const TimetableFolders = ({ title, timetables, onEdit, onDelete, onRefresh, onClearAll }) => (
  <div className="timetable-folders">
    <div className="timetable-folders-header">
      <h2 className="timetable-folders-title">{title}</h2>
      {onRefresh && (
        <button onClick={onRefresh} className="refresh-button" title="Refresh from database">
      ...
      {onClearAll && timetables.length > 0 && (
        <button onClick={onClearAll} className="refresh-button" style={{background:'#991b1b',borderColor:'#7f1d1d'}} title="Clear all timetables (local only)">
          <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div className="timetable-folders-container">
      {timetables.length > 0 ? (
        ...
        <p className="timetable-folders-empty">No timetables created yet. Create your first exam timetable!</p>
      )}
    </div>
  </div>
);
...
const TimetableFolder = ({ timetable: folder, onEdit, onDelete }) => {
  ...
  return (
    <div className="timetable-folder">
      <div className="timetable-folder-header" onClick={() => setIsExpanded(!isExpanded)}>
        <div className="timetable-folder-icon">
          <svg className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2H5a2 2 0 00-2-2V5a2 2 0 012-2h6l2 2h6a2 2 0 012 2z" />
          </svg>
        </div>
        <div className="timetable-folder-info">
          ...
          <p className="timetable-folder-meta">
            {getTimetableCount()} timetable(s) • {getExamCount()} exams • Created: {folder.createdAt ? formatDate(folder.createdAt) : 'Unknown'}
          </p>
        </div>
      ...
      {isExpanded && folder.timetables && folder.timetables.length > 0 && (
        <div className="timetable-folder-content">
          {folder.timetables.map((timetable, timetableIndex) => (
            <div key={timetable.id} className="timetable-preview" style={{ marginBottom: '20px' }}>
              <h4 style={{ color: '#fbbf24', marginBottom: '10px' }}>
                {timetable.tableName} 
                <span style={{ color: '#9ca3af', fontSize: '0.9em', marginLeft: '10px' }}>
              ...
              <table className="preview-table">
                ...
                <tbody>
                  {timetable.timetable && timetable.timetable.slice(0, 3).map((day, index) => (
              ...
              {getExamCount() > 6 && (
                <p className="preview-more">... and more exams</p>
              )}
            </div>
          ))}
        </div>
          ...
          {exams.map((exam) => (
            <li key={exam.id} className="exam-item">
              <div className="exam-item-info">
                <h3>{exam.title}</h3>
                <p>Duration: {exam.duration} minutes | Credit Points: {exam.creditPoints}</p>
              </div>
...
const CreateExamForm = ({ onCreateExam, editingTimetable = null, onUpdateTimetable, dispatch }) => {
  const [numSubjects, setNumSubjects] = useState(1);
  const [subjects, setSubjects] = useState([{ id: 1, name: '', duration: '180', credits: '' }]);
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');
  const [dayGap, setDayGap] = useState(2); // Default max 2 days gap between exams
  const [tableName, setTableName] = useState('');
  const [generatedTimetable, timetableDispatch] = useReducer(timetableReducer, []);
  ...
  useEffect(() => {
    if (editingTimetable) {
      ...
      editingTimetable.timetable.forEach((day) => {
        if (day.morning && !extractedSubjects.find(s => s.name === day.morning.name)) {
        ...
        if (day.afternoon && !extractedSubjects.find(s => s.name === day.afternoon.name)) {
          extractedSubjects.push({
            id: extractedSubjects.length + 1,
      ...
      timetableDispatch({ type: 'SET_TIMETABLE', payload: editingTimetable.timetable });
    }
  }, [editingTimetable]);
  ...
  const handleNumSubjectsChange = (e) => {
    ...
    const newSubjects = Array.from({ length: count }, (_, i) => {
      const existing = subjects[i] || { name: '', duration: '180', credits: '' };
  ...
  const generateTimetable = async (e) => {
    ...
    for (let i = 0; i < validSubjects.length; i++) {
      ...
      if (currentDate > endDateTime) {
        setErrorMessage(`Warning: Could only schedule ${i} out of ${validSubjects.length} subjects within the date range. Consider extending the end date or increasing the max day gap.`);
        break;
      }
      ...
      const timeSlot = useAfternoon ? '12:30 PM - 3:30 PM' : '9:00 AM - 12:00 PM';
      ...
      if (!dayData[session]) {
        dayData[session] = {
          ...
          time: timeSlot,
          session: sessionName,
          code: `${validSubjects[i].name.slice(0, 3).toUpperCase()}${String(i + 1).padStart(3, '0')}`
        };
      } else {
        // If preferred session is taken, use the other session
        const altSession = session === 'morning' ? 'afternoon' : 'morning';
        const altSessionName = altSession === 'afternoon' ? 'Afternoon' : 'Forenoon';
        const altTimeSlot = altSession === 'afternoon' ? '12:30 PM - 3:30 PM' : '9:00 AM - 12:00 PM';
    ...
    const timetableObject = {
      ...
      createdAt: isEditMode && editingTimetable ? editingTimetable.createdAt : new Date().toISOString(),
      updatedAt: new Date().toISOString()
    };

    // Store in localStorage for persistence (ensure single instance per tableName)
    const storedTimetables = JSON.parse(localStorage.getItem('examTimetables') || '[]');
    const existingIndexById = storedTimetables.findIndex(t => t.id === timetableObject.id);
    const existingIndexByName = storedTimetables.findIndex(t => t.tableName && t.tableName.toLowerCase() === timetableObject.tableName.toLowerCase());

    if (isEditMode && editingTimetable) {
      if (existingIndexById !== -1) {
        storedTimetables[existingIndexById] = timetableObject;
      }
      onUpdateTimetable && onUpdateTimetable(timetableObject);
    } else {
      if (existingIndexByName !== -1) {
        storedTimetables[existingIndexByName] = timetableObject; // Replace existing with same name
      } else {
        storedTimetables.push(timetableObject);
      }
      onCreateExam && onCreateExam(timetableObject);
    }
    localStorage.setItem('examTimetables', JSON.stringify(storedTimetables));

    // Backend API calls removed due to CORS issues
    // The timetable is generated and displayed, but not automatically saved to backend
    // Users can use the "Store to Database" button if backend CORS is resolved
  };

  const handleCellEdit = (dayIndex, session, field, value) => {
    timetableDispatch({ type: 'UPDATE_CELL', payload: { dayIndex, session, field, value } });
  };
  ...
  const handleDownloadWord = () => {
            ...
            .map(
              (dayData) => {
                ...
                <tr style="background-color: #1f2937; color: #d1d5db;">
                  <td style="padding: 12px; border: 1px solid #4b5563; text-align: center;">${dayData.date}</td>
                  <td style="padding: 12px; border: 1px solid #4b5563; text-align: center;">${dayData.morning.session}</td>
                  <td style="padding: 12px; border: 1px solid #4b5563; text-align: center;">${dayData.day}</td>
                  <td style="padding: 12px; border: 1px solid #4b5563; text-align: center;">${dayData.morning.time}</td>
                  <td style="padding: 12px; border: 1px solid #4b5563; text-align: center;">${dayData.morning.code || ''}</td>
                  <td style="padding: 12px; border: 1px solid #4b5563; text-align: left;">${dayData.morning.name || ''}</td>
                  <td style="padding: 12px; border: 1px solid #4b5563; text-align: center;">${dayData.morning.duration || '180'} mins</td>
                  <td style="padding: 12px; border: 1px solid #4b5563; text-align: center;">${dayData.morning.credits || ''}</td>
                </tr>
              `;
                }
                ...
                <tr style="background-color: #374151; color: #d1d5db;">
                  <td style="padding: 12px; border: 1px solid #4b5563; text-align: center;">${dayData.date}</td>
                  <td style="padding: 12px; border: 1px solid #4b5563; text-align: center;">${dayData.afternoon.session}</td>
                  <td style="padding: 12px; border: 1px solid #4b5563; text-align: center;">${dayData.day}</td>
                  <td style="padding: 12px; border: 1px solid #4b5563; text-align: center;">${dayData.afternoon.time}</td>
                  <td style="padding: 12px; border: 1px solid #4b5563; text-align: center;">${dayData.afternoon.code || ''}</td>
                  <td style="padding: 12px; border: 1px solid #4b5563; text-align: left;">${dayData.afternoon.name || ''}</td>
                  <td style="padding: 12px; border: 1px solid #4b5563; text-align: center;">${dayData.afternoon.duration || '180'} mins</td>
                  <td style="padding: 12px; border: 1px solid #4b5563; text-align: center;">${dayData.afternoon.credits || ''}</td>
                </tr>
              `;
                }
                return rows;
    ...
    `;
    const header = "<html xmlns:o='urn:schemas-microsoft-com:office:office' xmlns:w='urn:schemas-microsoft-com:office:word' xmlns='http://www.w3.org/TR/REC-html40'><head><meta charset='utf-8'><title>Exam Timetable</title></head><body>";
    ...
    try {
      if (isEditMode && editingTimetable) {
        ...
        if (result && result.success) {
          ...
          alert(result ? result.message : 'Failed to store timetable to database');
        }
        return;
      }
    } catch (error) {
      ...
      <div className="create-exam-container">
        <form onSubmit={generateTimetable}>
          ...
          {subjects.map((subject, index) => (
            <div key={subject.id} className="subject-grid">
              ...
              <div className="subject-input-group">
                <label className="subject-input-label">Exam Duration (minutes)</label>
                <input
                  type="number"
                  name="duration"
          ...
          <div className="scheduling-info">
            ...
            <ul>
              <li><strong>Priority:</strong> Subjects are scheduled by highest credit points first</li>
              <li><strong>Random Gaps:</strong> 1 to {dayGap} days gap between exams (randomly assigned)</li>
              <li><strong>Sessions:</strong> Morning (9:00 AM - 12:00 PM) and Afternoon (12:30 PM - 3:30 PM)</li>
              <li><strong>Weekends:</strong> Automatically skipped</li>
            </ul>
          </div>
        ...
        {generatedTimetable.length > 0 && (
          <div className="timetable-section">
            ...
            <p className="timetable-instruction">Click on any cell to edit the content.</p>
                    ...
                    <React.Fragment key={`day-${dayIndex}`}>
                      {dayData.morning && (
                        <tr key={`${dayIndex}-morning`} className="table-row">
                          <td className="table-cell date-cell">{dayData.date}</td>
                          <td className="table-cell session-cell">{dayData.morning.session}</td>
                          <td className="table-cell day-cell">{dayData.day}</td>
                          <td className="table-cell time-cell">{dayData.morning.time}</td>
                          <td className="table-cell code-cell">
                            {editingCell === `${dayIndex}-morning-code` ? (
                              <input
                                type="text"
                                value={dayData.morning.code || ''}
                                onChange={(e) => handleCellEdit(dayIndex, 'morning', 'code', e.target.value)}
                              ...
                              <span
                                onClick={() => handleCellClick(dayIndex, 'morning', 'code')}
                          ...
                          <td className="table-cell name-cell">
                            {editingCell === `${dayIndex}-morning-name` ? (
                              <input
                                ...
                                onChange={(e) => handleCellEdit(dayIndex, 'morning', 'name', e.target.value)}
                                onBlur={handleCellBlur}
                                onKeyPress={(e) => e.key === 'Enter' && handleCellBlur()}
                                className="cell-input"
                                autoFocus
                              ...
                              <span
                                onClick={() => handleCellClick(dayIndex, 'morning', 'name')}
                                className="editable-cell"
                              >
                          ...
                          <td className="table-cell duration-cell">
                            {editingCell === `${dayIndex}-morning-duration` ? (
                              <input
                                ...
                                onChange={(e) => handleCellEdit(dayIndex, 'morning', 'duration', e.target.value)}
                                onBlur={handleCellBlur}
                                onKeyPress={(e) => e.key === 'Enter' && handleCellBlur()}
                                className="cell-input"
                                autoFocus
                              ...
                              <span
                                onClick={() => handleCellClick(dayIndex, 'morning', 'duration')}
                                className="editable-cell"
                              >
                          ...
                          <td className="table-cell credits-cell">
                            {editingCell === `${dayIndex}-morning-credits` ? (
                              <input
                                ...
                                onChange={(e) => handleCellEdit(dayIndex, 'morning', 'credits', e.target.value)}
                                onBlur={handleCellBlur}
                                onKeyPress={(e) => e.key === 'Enter' && handleCellBlur()}
                                className="cell-input"
                                autoFocus
                              ...
                              <span
                                onClick={() => handleCellClick(dayIndex, 'morning', 'credits')}
                                className="editable-cell"
                              >
                      ...
                      {dayData.afternoon && (
                        <tr key={`${dayIndex}-afternoon`} className="table-row">
                          <td className="table-cell date-cell">{dayData.date}</td>
                          <td className="table-cell session-cell">{dayData.afternoon.session}</td>
                          <td className="table-cell day-cell">{dayData.day}</td>
                          <td className="table-cell time-cell">{dayData.afternoon.time}</td>
                          <td className="table-cell code-cell">
                            {editingCell === `${dayIndex}-afternoon-code` ? (
                              <input
                                type="text"
                                value={dayData.afternoon.code || ''}
                                onChange={(e) => handleCellEdit(dayIndex, 'afternoon', 'code', e.target.value)}
                                onBlur={handleCellBlur}
                                onKeyPress={(e) => e.key === 'Enter' && handleCellBlur()}
                                className="cell-input"
                                autoFocus
                              ...
                              <span
                                onClick={() => handleCellClick(dayIndex, 'afternoon', 'code')}
                                className="editable-cell"
                              >
                          ...
                          <td className="table-cell name-cell">
                            {editingCell === `${dayIndex}-afternoon-name` ? (
                              <input
                                ...
                                onChange={(e) => handleCellEdit(dayIndex, 'afternoon', 'name', e.target.value)}
                                onBlur={handleCellBlur}
                                onKeyPress={(e) => e.key === 'Enter' && handleCellBlur()}
                                className="cell-input"
                                autoFocus
                              ...
                              <span
                                onClick={() => handleCellClick(dayIndex, 'afternoon', 'name')}
                                className="editable-cell"
                              >
                          ...
                          <td className="table-cell duration-cell">
                            {editingCell === `${dayIndex}-afternoon-duration` ? (
                              <input
                                type="text"
                                value={dayData.afternoon.duration || '180'}
                                onChange={(e) => handleCellEdit(dayIndex, 'afternoon', 'duration', e.target.value)}
                                onBlur={handleCellBlur}
                                onKeyPress={(e) => e.key === 'Enter' && handleCellBlur()}
                                className="cell-input"
                                autoFocus
                              ...
                              <span
                                onClick={() => handleCellClick(dayIndex, 'afternoon', 'duration')}
                                className="editable-cell"
                              >
                          ...
                          <td className="table-cell credits-cell">
                            {editingCell === `${dayIndex}-afternoon-credits` ? (
                              <input
                                ...
                                onChange={(e) => handleCellEdit(dayIndex, 'afternoon', 'credits', e.target.value)}
                                onBlur={handleCellBlur}
                                onKeyPress={(e) => e.key === 'Enter' && handleCellBlur()}
                                className="cell-input"
                                autoFocus
                              ...
                              <span
                                onClick={() => handleCellClick(dayIndex, 'afternoon', 'credits')}
                                className="editable-cell"
                              >
            ...
            <div className="download-section">
              <button onClick={handleStoreToDatabase} className="download-button download-button-store">
                Store to Database
              </button>
              <button onClick={handleDownloadWord} className="download-button download-button-word">
                Download Word
              </button>
              <button onClick={handleDownloadImage} className="download-button download-button-image">
                Download Image
              </button>
              <button onClick={handleDownloadPDF} className="download-button download-button-pdf">
                Download PDF
              </button>
            </div>
          </div>
